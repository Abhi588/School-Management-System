...
public class PLUSTWO extends JInternalFrame implements ActionListener {
    ...
    public PLUSTWO(String rno, String title, int semno) {
        ...
        btnClose = new JButton("close", new ImageIcon(getClass().getClassLoader().getResource("Images/cancel.png")));
    ...
    private void loadSubjectNames() {
        try {
            String sql = "Select SubjectName from SubjectAllocation where Semester=" + semester + " and Batch='PLUS TWO'";
            Connection con = Settings.getDBConnection();
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
